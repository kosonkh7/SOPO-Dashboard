...
- **데이터**: 일자별로 집계된 물류센터 / 품목군별 물동량 데이터 (6년간 서울시 생활물류 공공 데이터를 병합 및 전처리하여 이용)
- 🎯 **목표**: 
  - MFC 운영 효율화를 위한 EDA & ML을 이용한 수요 예측 기반 인사이트 제공
  - 통계 기반 이상 탐지 및 원인 자동 분석 기능 포함
  - 관리자 친화적인 편리한 UI 설계
...
```
▶︎ 2. FastAPI 라우터(중계) 서버 실행
```bash
uvicorn main:app --port 8005 --reload
```
▶︎ 3. Spring Boot 웹에 iframe 삽입
```html
...
- 필터링 옵션 세분화 (원하는 정보 편하기 확인 가능하도록)

- 주간 리포트 자동 생성 및 이메일 전송 기능 (실무 관점에서 필요하다고 느낌)
